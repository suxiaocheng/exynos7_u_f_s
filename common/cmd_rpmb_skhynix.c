...
static int ufs_rpmb_commands(int dev_num, struct rpmb_packet *packet)
{
	...
	case 3:
		...
		dump_packet((u8 *) packet, DATA_SIZE * blk_cnt);
		...
		dump_packet((u8 *) buf + (blk_cnt - 1), RPMB_SIZE);
...
U_BOOT_CMD(rpmb, 7, 0, do_rpmb,
	   ...
	   "sub commands list\n"
	   "\trpmb write {device_number} key {addr} - Write authentication KEY\n"
