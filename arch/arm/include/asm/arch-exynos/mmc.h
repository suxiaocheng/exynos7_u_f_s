...
static inline unsigned int s5p_mmc_init(int index, int bus_width)
{
	...
	unsigned int base = (index < 2) ? samsung_get_base_mmc() + (0x10000 * index) : \
				samsung_get_base_mmc() - (0x1E0000);
#else
	unsigned int base = samsung_get_base_mmc() + (0x10000 * index);
#endif

#ifndef CONFIG_CPU_EXYNOS7580
        unsigned int mshc_ver_main_id = readl(base + MSHC_VERID) >> 16;
//	printf("s5p_mmc_init(in_4) - base = 0x%08X\n", base);
