...
static void IIC_DISP_EWrite_chg(unsigned char ChipId,
...
static void IIC_DISP_ERead_chg(unsigned char ChipId,
		unsigned char IicAddr, unsigned char *IicData)
{
	unsigned long i, reg;
...
void pmic_mic23156_chg_vq1v1_init(unsigned int val)
{
	unsigned char reg_en = 0;

	IIC_DISP_ESetport();

	IIC_DISP_EWrite_chg(MIC23156_W_ADDR, MIC23156_BUCK1, val);

	IIC_DISP_ERead_chg(MIC23156_R_ADDR, MIC23156_CTRL, &reg_en);
	reg_en |= (BUCK_EN);
	IIC_DISP_EWrite_chg(MIC23156_W_ADDR, MIC23156_CTRL, reg_en);
}
