...
#if 0
#define	 TZASC_BASE 0x10100000
void set_non_secure_memory(void)
{
#if defined(CONFIG_SMC_CMD)
	unsigned int i;
	reg_arr_t reg_arr;

	reg_arr.set0.addr = ((u64)TZASC_BASE);
	reg_arr.set0.val = 0;
	set_secure_reg((u64)&reg_arr, 1);

	for (i=0x10080000;i<=0x100A0000;i+= 0x10000)
	{
		reg_arr.set0.addr = ((u64)(i+0x804));
		reg_arr.set0.val = 1;
		reg_arr.set1.addr = ((u64)(i+0x810));
		reg_arr.set1.val = 1;
		reg_arr.set2.addr = ((u64)(i+0x81c));
		reg_arr.set2.val = 1;
		reg_arr.set3.addr = ((u64)(i+0x828));
		reg_arr.set3.val = 1;

		set_secure_reg((u64)&reg_arr, 4);
	}

	for (i=0x100d0000;i<=0x10190000;i+= 0x10000)
	{
		reg_arr.set0.addr = ((u64)(i+0x804));
		reg_arr.set0.val = 1;
		reg_arr.set1.addr = ((u64)(i+0x810));
...
#else
#if 0
	__REG32(TZASC_BASE+0x804) =(u32)1;
	__REG32(TZASC_BASE+0x810) =(u32)1;
	__REG32(TZASC_BASE+0x81C) = (u32)1;	
	__REG32(TZASC_BASE+0x828) = (u32)1;	

	__REG32(TZASC_BASE) = (u32)0;
	__REG32(TZASC_BASE+0x10000) = (u32)0;
	__REG32(TZASC_BASE+0x20000) = (u32)0;
#endif
#endif
 }
#endif
