...
u32 re_err(void)
{
	printf("ASV: CAL is working wrong. \n");
	return 0;
}
#endif
static void cal_print_asv_info(void)
{
	printf("(ASV_TBL_BASE+0x00)[11:0]  bigcpu_asv_group         = %d\n", gasv_table_info.bigcpu_asv_group);
	printf("(ASV_TBL_BASE+0x00)[15:12] bigcpu_ssa0              = %d\n", gasv_table_info.bigcpu_ssa0);
	printf("(ASV_TBL_BASE+0x00)[27:16] littlecpu_asv_group      = %d\n", gasv_table_info.littlecpu_asv_group);
	printf("(ASV_TBL_BASE+0x00)[31:28] littlecpu_ssa0           = %d\n", gasv_table_info.littlecpu_ssa0);

	printf("(ASV_TBL_BASE+0x04)[11:0]  g3d_asv_group            = %d\n", gasv_table_info.g3d_asv_group);
	printf("(ASV_TBL_BASE+0x04)[15:12] g3d_ssa0                 = %d\n", gasv_table_info.g3d_ssa0);
	printf("(ASV_TBL_BASE+0x04)[27:16] mif_asv_group            = %d\n", gasv_table_info.mif_asv_group);
	printf("(ASV_TBL_BASE+0x04)[31:28] mif_ssa0                 = %d\n", gasv_table_info.mif_ssa0);

	printf("(ASV_TBL_BASE+0x08)[11:0]  int_asv_group            = %d\n", gasv_table_info.int_asv_group);
	printf("(ASV_TBL_BASE+0x08)[15:12] int_ssa0                 = %d\n", gasv_table_info.int_ssa0);
	printf("(ASV_TBL_BASE+0x08)[27:16] cam_disp_asv_group       = %d\n", gasv_table_info.cam_disp_asv_group);
	printf("(ASV_TBL_BASE+0x08)[31:28] cam_disp_ssa0            = %d\n", gasv_table_info.cam_disp_ssa0);

	printf("(ASV_TBL_BASE+0x0C)[3:0]   dvfs_asv_table_version   = %d\n", gasv_table_info.dvfs_asv_table_version);
	printf("(ASV_TBL_BASE+0x0C)[4]     asv_group_type           = %d\n", gasv_table_info.asv_group_type);
	printf("(ASV_TBL_BASE+0x0C)[7:5]   reserved01               = %d\n", gasv_table_info.reserved01);
	printf("(ASV_TBL_BASE+0x0C)[8]     shift_type               = %d\n", gasv_table_info.shift_type);
	printf("(ASV_TBL_BASE+0x0C)[9]     ssa1_enable               = %d\n", gasv_table_info.ssa1_enable);
	printf("(ASV_TBL_BASE+0x0C)[10]    ssa0_enable             = %d\n", gasv_table_info.ssa0_enable);
	printf("(ASV_TBL_BASE+0x0C)[15:11] reserved02               = %d\n", gasv_table_info.reserved02);
	printf("(ASV_TBL_BASE+0x0C)[16]    asv_method               = %d\n", gasv_table_info.asv_method);
	printf("(ASV_TBL_BASE+0x0C)[31:17] reserved03               = %d\n", gasv_table_info.reserved03);

	printf("(ASV_TBL_BASE+0x10)[3:0]   main_asv_group           = %d\n", gasv_table_info.main_asv_group);
	printf("(ASV_TBL_BASE+0x10)[7:4]   main_asv_ssa             = %s%d\n",(gasv_table_info.main_asv_ssa_minus_sign==1) ? "-":"",gasv_table_info.main_asv_ssa);
	printf("(ASV_TBL_BASE+0x10)[11:8]  bigcpu_ssa1              = %d\n", gasv_table_info.bigcpu_ssa1);
	printf("(ASV_TBL_BASE+0x10)[15:12] littlecpu_ssa1           = %d\n", gasv_table_info.littlecpu_ssa1);
	printf("(ASV_TBL_BASE+0x10)[19:16] g3d_ssa1                 = %d\n", gasv_table_info.g3d_ssa1);
	printf("(ASV_TBL_BASE+0x10)[23:20] mif_ssa1                 = %d\n", gasv_table_info.mif_ssa1);
	printf("(ASV_TBL_BASE+0x10)[27:24] int_ssa1                 = %d\n", gasv_table_info.int_ssa1);
	printf("(ASV_TBL_BASE+0x10)[31:28] cam_disp_ssa1            = %d\n", gasv_table_info.cam_disp_ssa1);

	printf("(ASV_TBL_BASE+0x14)[8:0]   bigcpu_ssa_ema           = %d\n", gasv_table_info.bigcpu_ssa_ema);
	printf("(ASV_TBL_BASE+0x14)[17:9]  littlecpu_ssa_ema        = %d\n", gasv_table_info.littlecpu_ssa_ema);
	printf("(ASV_TBL_BASE+0x14)[26:18] g3d_ssa_ema              = %d\n", gasv_table_info.g3d_ssa_ema);
	printf("(ASV_TBL_BASE+0x14)[31:27] reserved04               = %d\n", gasv_table_info.reserved04);
}
...
{
	return readl(CHIPID_ASV_INFO + 0x01C0) & 0xff;;
}

u32 cal_get_hpm(void)
{
	return (readl(CHIPID_ASV_INFO + 0x01C4) >> 24) & 0xff;
}
...
u32 cal_get_volt(u32 id, s32 level)
{
	...
	if (p_table == NULL) {
		debug("%s : voltae table pointer is NULL\n", __func__);
		return 0;
	}
