...
static void display_pmic_info(void)
{
	...
	IIC0_EWrite(S2MPS15_ADDR, LDO25_CTRL, 0xFC);	// 3.3V
#endif
	...
	/* power control for BTYPE7420 SD/MMC */
	/* enable LDO25 */
	IIC0_EWrite(S2MPS15_ADDR, LDO25_CTRL, &read_ldo_dvs1);
...
#if defined(CONFIG_PMIC_ACT8892_OLED_FHD)
extern void oled_pmic_init(void);
#endif
...
#define regwrite(addr, value, mask, shift)	writel((readl(addr)&(~((mask)<<(shift))))|((value)<<(shift)), (addr))
void eMMC_UFS_Power_On(void)
{
#ifdef CONFIG_EMMC_UFS_POWER_ON
	regwrite(0x14870120, 1, 0xF, 16);
	regwrite(0x14870124, 1, 1, 4);
	mdelay(50);
#endif
}
...
int board_late_init(void)
{
	...
	/* check reset status for ramdump */
		...
		printf("CONFIG_RAMDUMP_MODE\n");	
		run_command("ramdump", 0);
		/*run_command("fastboot", 0); */ // by bskim
	}
