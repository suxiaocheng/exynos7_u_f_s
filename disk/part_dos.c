				 ...
				 int which_part, disk_partition_t *info)
{
	...
	for (i = 0; i < 4; i++, pt++) {
		...
		if (((pt->boot_ind & ~0x80) == 0) &&
			...
			info->blksz_p = dev_desc->blksz;
			info->start_p = ext_part_sector + le32_to_int (pt->start4);
			info->size_p  = le32_to_int (pt->size4);

			info->blksz = 512;
			info->start = info->start_p*(dev_desc->blksz/info->blksz);
			info->size  = info->size_p*(dev_desc->blksz/info->blksz);
#else
			info->blksz = 512;
...
#if defined(CONFIG_SCSI_UFS) || defined(CONFIG_MMC)
			if (which_part == USER_DATA_PART)
				info->size =  info->size - FMP_CRYPTO_FOOTER - FMP_SELF_TEST;
#endif
#endif
