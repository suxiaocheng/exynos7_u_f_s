...
static const char ifname_part[][16] =
{
	"mmc",
...
static const char devname_part[][16] =
{
	"MMC",
#if defined(CONFIG_CMD_SCSI)
	"UFS",
...
char *get_boot_dev_name(int type)
{
	switch (exynos_boot_mode()) {
#if defined(CONFIG_CMD_SCSI)
	case BOOT_UFS:
		return (type?devname_part[1]:ifname_part[1]);
		break;
#endif
	case BOOT_MMCSD:
	case BOOT_EMMC:
	case BOOT_EMMC_4_4:
		return (type?devname_part[0]:ifname_part[0]);
		break;
	default:
		return (type?devname_part[0]:NULL);
	}
	return NULL;
}
