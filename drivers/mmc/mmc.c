...
static int mmc_bounce_buffer_start(struct mmc_data *backup, struct mmc_data *orig)
{
	ulong origlen, len;
	void *buffer;
...
static void mmc_bounce_buffer_stop(struct mmc_data *backup, struct mmc_data *orig)
{
	ulong len;
...
#else
static inline int mmc_bounce_buffer_start(struct mmc_data *backup, struct mmc_data *orig)
{
	return 0;
}

static inline void mmc_bounce_buffer_stop(struct mmc_data *backup, struct mmc_data *orig)
{
}
#endif
...
static unsigned long mmc_berase(int dev_num, unsigned long start, lbaint_t blkcnt)
{
	int err = 0;
	 ...
	 */
	if (!IS_SD(mmc) && start == 0 && ((mmc->capacity / mmc->write_bl_len) / 2) < blkcnt) {
		if (mmc->sec_feature_support & EXT_CSD_SEC_SANITIZE)
	...
	if (sec && mmc->sec_feature_support & EXT_CSD_SEC_SANITIZE) {
		err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_SANITIZE_START, 1);
		if (err)
...
static ulong mmc_write_blocks(struct mmc *mmc, ulong start, lbaint_t blkcnt, const void *src)
{
	struct mmc_cmd cmd;
...
int mmc_startup(struct mmc *mmc)
{
	...
	if (!IS_SD(mmc) && (mmc->version >= MMC_VERSION_4)) {
		...
		mmc->boot_size_multi = ext_csd[EXT_CSD_BOOT_SIZE_MULTI] * 128 * 1024;
	...
	if (IS_SD(mmc)) {
		...
		/* check mmc bus width */
		if ((mmc->host_caps & MMC_MODE_MASK_WIDTH_BITS) == MMC_MODE_MASK_WIDTH_BITS)
...
int mmc_send_if_cond(struct mmc *mmc)
{
	...
	if (err)
	{
		//printf("MMC if cond error !!!\n");
		return err;
	}
	
	if ((cmd.response[0] & 0xff) != 0xaa)
	{
		//printf("MMC unusable error !!!!\n");
		return UNUSABLE_ERR;
	}
	else
		mmc->version = SD_VERSION_2;
...
int mmc_init(struct mmc *mmc)
{
	...
	if (mmc->has_init)
	{
		//printf("MMC already initialized !! dev = %d\n", mmc_getcd(mmc));
		return 0;
	}
