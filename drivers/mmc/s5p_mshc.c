...
#if defined(CONFIG_CPU_EXYNOS5433) || defined(CONFIG_CPU_EXYNOS7420) || \
	defined(CONFIG_CPU_EXYNOS7580)
static void mshci_set_mdma_desc(u8 *desc_vir, u8 *desc_phy,
				u32 des0, u32 des2, u32 des4)
{
	((struct mshci_idmac *)(desc_vir))->des0 = des0;
	((struct mshci_idmac *)(desc_vir))->des1 = 0;
	((struct mshci_idmac *)(desc_vir))->des2 = des2;
	((struct mshci_idmac *)(desc_vir))->des3 = 0;
	((struct mshci_idmac *)(desc_vir))->des4 = des4;
	((struct mshci_idmac *)(desc_vir))->des5 = 0;
	((struct mshci_idmac *)(desc_vir))->des6 = (u32)desc_phy +
					sizeof(struct mshci_idmac);
	((struct mshci_idmac *)(desc_vir))->des7 = 0;

	((struct mshci_idmac *)(desc_vir))->des8 = 0;
	((struct mshci_idmac *)(desc_vir))->des9 = 0;
	((struct mshci_idmac *)(desc_vir))->des10 = 0;
	((struct mshci_idmac *)(desc_vir))->des11 = 0;
	((struct mshci_idmac *)(desc_vir))->des12[0] = 0;
	((struct mshci_idmac *)(desc_vir))->des12[1] = 0;
	((struct mshci_idmac *)(desc_vir))->des12[2] = 0;
	((struct mshci_idmac *)(desc_vir))->des12[3] = 0;
}
#else
static void mshci_set_mdma_desc(u8 *desc_vir, u8 *desc_phy,
...
static void mshci_prepare_data(struct mshci_host *host, struct mmc_data *data)
{
...
#if defined(CONFIG_CPU_EXYNOS7420_EVT1) || defined(CONFIG_CPU_EXYNOS7580)
	u32 fifo_val;
	 ...
	 * IDMA reset
	 */
	bmod = mshci_readl(host, MSHCI_BMOD);
	bmod |= BMOD_IDMAC_RESET;
	mshci_writel(host, bmod, MSHCI_BMOD);
			...
			struct mmc_data *data)
{
...
#if defined(CONFIG_CPU_EXYNOS5433)
		dbg("BLOCKS: %d, SIZE: %d\n",data->blocks, data->blocksize);
		if (data->blocksize == 8 && data->blocks == 1) {
			mshci_writel(host, AXI_BURST_LEN(0), MSHCI_AXI_BUSRT_LENGTH);
		} else {
			u32 sector_size;

			mshci_writel(host, AXI_BURST_LEN(3),
					MSHCI_AXI_BUSRT_LENGTH);
			sector_size = mshci_readl(host, MSHCI_SECTOR_NUM_INC);
			sector_size &= ~SECTOR_SIZE_MASK;
			sector_size |= SECTOR_SIZE(data->blocksize);
			mshci_writel(host, sector_size, MSHCI_SECTOR_NUM_INC);
	...
	if (mask & INTMSK_RTO) {
		if (((cmd->cmdidx == 8 || cmd->cmdidx == 41 ||
			cmd->cmdidx == 55)) == 0) {
			//printf("[ERROR] response timeout error : %08x cmd %d %s\n",
			//mask, cmd->cmdidx, mmc->name);
		}
			return TIMEOUT;
...
#if defined(CONFIG_DEBUG_MMC_SD)		
		printf("Input CLK is too high. CLK: %dMHz\n", clock / MHZ);
		clock = host->max_clk;
		printf("Set CLK to %dMHz\n", clock / MHZ);
#else
...
#if defined(CONFIG_DEBUG_MMC_SD)
	printf("sclk_mmc: %dKHz\n", sclk_mmc / KHZ);
	printf("Phase Shift CLK: %dKHz\n", (sclk_mmc / host->phase_devide) / KHZ);
#else
	dbg("sclk_mmc: %dKHz\n", sclk_mmc / KHZ);
...
#if defined(CONFIG_DEBUG_MMC_SD)
	printf("%s(in) host = 0x%08X ioaddr = 0x%08X<--\n", __func__, (unsigned int)host, host->ioaddr);
#endif	
...
#if defined(CONFIG_CPU_EXYNOS7420_EVT1) || defined(CONFIG_CPU_EXYNOS7580)
		mpsec = mshci_readl(host, MSHCI_MPSECURITY);
		mpsec &= ~(MMC_SFR_PROT_ON);
		mshci_writel(host, mpsec, MSHCI_MPSECURITY);
#endif
	}
#endif
...
int s5p_mshc_init(unsigned int regbase, unsigned int channel)
{
	struct mmc *mmc;
#if defined(CONFIG_DEBUG_MMC_SD)
	printf("%s(in) regbase = 0x%08X channel = %d<--\n", __func__, regbase, channel);
#endif
