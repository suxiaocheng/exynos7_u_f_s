...
#if defined(CONFIG_ARCH_EXYNOS5) || defined(CONFIG_ARCH_EXYNOS7)
#include <command.h>
#include <asm/errno.h>
#include <asm/io.h>
...
void exynoy_usb_phy_on(void)
{
	...
	writel((0x1 << 28), USBDEVICE3_PHYCLK_SEL);
#endif
}
...
static void exynos_usb_init_phy(void)
{
	...
	/*
	 * Set PHY tune for Receive sensitivity
	 * In Exynos5430 & Exynos5433, it is needed.
	 * sqrxtune[8:6] 3'b111 : -20%
	 */
#if defined(CONFIG_CPU_EXYNOS5430) || defined(CONFIG_CPU_EXYNOS5433)
	writel(readl(EXYNOS_PHY_PARAM0) | (0x7<<6), EXYNOS_PHY_PARAM0);
#endif
	...
	/*
	 * If the version of DRD LINK Controller is equal to or higher than 2.50a,
	 * the register for UTMI Clock Selection is PHY_PIPE.
	 * PHYPIPE[4]: PHY Clock = 0, Free Clock = 1
	 * If not, PHY_BATCHG is used instead.
	 * PHY CLK Mux. (1<<2) : w_FREECLK, (0<<2) : w_PHYCLK
	 */
	if (readl(rGSNPSID) >= EXYNOS_DRD_REVISION_250A)
		writel(readl(EXYNOS_PHY_PIPE) | (0x1<<4), EXYNOS_PHY_PIPE);
	else
		writel(0x00000004, EXYNOS_PHY_BATCHG);
...
static void exynos_usb_phy_crport_handshake(usb3_phy_reg0_t *phy_reg0, u32 exp)
{
...
static void exynos_phy_crport_ctrl(u16 addr, u16 data)
{
	...
	exynos_usb_phy_crport_handshake(&phy_reg0, 0x1);
	phy_reg0.b.cr_cap_addr = 0;
	exynos_usb_phy_crport_handshake(&phy_reg0, 0x0);
...
static u32 exynos_usb_malloc(u32 uSize, u32 uAlign)
{
	...
	memset((void *)uAddr, 0, uSize + uAlign);
	uTemp2 = uAddr % uAlign;
...
int exynos_usb_wait_cable_insert(void)
{
	...
	char ch;
	int ret = -1;
#if (defined(CONFIG_CPU_EXYNOS5250) || defined(CONFIG_CPU_EXYNOS5260) || defined(CONFIG_CPU_EXYNOS5430) || defined(CONFIG_CPU_EXYNOS7420))
	if(oUsbDev3.m_cable != CONNECTED) {
